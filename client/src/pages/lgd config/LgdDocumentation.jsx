...
function LgdDocumentation() {
  ...
  const fetchVersions = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/lgd-documentation/versions/all`);
  ...
  const handleView = (versionId) => {
    navigate(`/lgd-documentation/${versionId}`);
  };

  const handleCreateNew = () => {
    navigate('/create-lgd-documentation');
  };
  ...
  const handleAuthorize = async (versionId) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.post(`${API_URL}/lgd-documentation/${versionId}/authorize`);
